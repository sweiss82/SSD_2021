    ...
    if request.user.is_authenticated:
        if request.method == 'GET':
            ...
            bestellung = Medikamentenbestellung(medikamentenname=medikament, menge=menge,
                                                dosierung=dosierung, status=BestellungStatus.OFFEN.value,
                                                patient=patient, arzt=arzt, wirdAbgeholt=auswahl,
                                                apotheke=apotheke)
            bestellung.save()
    ...
    if request.user.is_authenticated:
        if request.method == 'GET':
            try:
                ...
                medikamentenbestellung = Medikamentenbestellung.objects.filter(arzt=arzt.id,
                                                                               status=BestellungStatus.OFFEN.value)
